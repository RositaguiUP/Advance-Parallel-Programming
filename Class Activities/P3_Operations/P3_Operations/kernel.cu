...
int main()
{
    ...
    int* phost_a, * phost_b, * phost_c, * phost_res;
    ...
    for (int i = 0; i < vectorSize; i++) {
        phost_a[i] = rand() % 255;
        phost_b[i] = rand() % 255;
        phost_c[i] = phost_a[i] + phost_b[i];
        // printf("\n%d) %d + %d = %d", i, phost_a[i], phost_b[i], phost_c[i]);
    }
    ...
    gpu_start = clock();
    addVectors << < gridDim, blockDim >> > (dev_a, dev_b, dev_c, size);
    ...
    cudaMemcpy(phost_res, dev_c, size, cudaMemcpyDeviceToHost);

    bool equal = true;
    for (int i = 0; i < vectorSize; i++) {
        if (phost_c[i] != phost_res[i]) {
            equal = false;
            printf("%d", i);
            break;
        }
